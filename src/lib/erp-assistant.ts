...
export type StructuredAssistantItem = {
  label: string;
  detail: string;
  href?: string;
};
...
export async function runErpAssistant(input: {
  ...
  const snapshot = await getDashboardSnapshot(input.scopedCompanyId);
  const [
    company,
    clients,
    suppliers,
    ...
    prisma.company.findUnique({
      where: { id: input.scopedCompanyId },
      select: {
        id: true,
        name: true,
        domain: true,
    ...
    prisma.client.findMany({
      where: { companyId: input.scopedCompanyId },
  ...
  const result = await runCompanyLlm({
    companyId: input.configCompanyId,
    message:
  ...
  await logAudit({
    companyId: input.scopedCompanyId,
    userId: input.actorUserId,
    action: "LLM_ASSISTANT_QUERY",
    entity: "llm_assistant",
    entityId: input.scopedCompanyId,
    metadata: {
